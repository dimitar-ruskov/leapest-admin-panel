...
import {
  CertificateInfo, CourseCategory, EnrollmentPolicyKeys, GeneralInfoField,
  IKeyValuePair, PublishedILTCourse,
  S3BucketData
...
@UntilDestroy()
export class IltCourseGeneralInfoComponent implements OnInit, OnChanges {
  @Select(IltCourseDetailsState.iltCourse)
  iltCourse$: Observable<DeferredResource<PublishedILTCourse> | null>;
  ...
  ngOnInit(): void {
    this.iltCourse$
      .pipe(
        filter((resource: DeferredResource<PublishedILTCourse>) => !resource.isPending && !!resource.response),
        map((resource: DeferredResource<PublishedILTCourse>) => resource.response),
        untilDestroyed(this),
      )
      .subscribe((selfPacedCourse: PublishedILTCourse) => {
        this.editLock = selfPacedCourse.editLock;
        this.iltCourse = selfPacedCourse;
        this.fields = this.prepareGeneralInfoFields(selfPacedCourse);
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.iltCourse) {
      const currentValue = changes.iltCourse.currentValue;

      if (currentValue) {
        this.fields = this.prepareGeneralInfoFields(currentValue);
      }
    }
  }
  ...
  private prepareGeneralInfoFields(course: PublishedILTCourse): GeneralInfoField[] {
    return [
      ...
      },
      {
        ...
        value: {
          content: course.lxpPrivate === true ? 'Private' : course.lxpPrivate === false ? 'Public' : 'N/A',
          description:
      ...
      },
      {
        ...
        value: {
          content: course.lxpRestrictUsers?.length ? course.lxpRestrictUsers.map((item) => item).join(', ') : 'N/A',
          contentType: 'html',
        },
        editable: false,
      },
      {
        ...
        value: {
          content: course.lxpRestrictGroups?.length
            ? course.lxpRestrictGroups.map((item) => item.value).join(', ')
            : 'N/A',
          contentType: 'html',
        },
        editable: false,

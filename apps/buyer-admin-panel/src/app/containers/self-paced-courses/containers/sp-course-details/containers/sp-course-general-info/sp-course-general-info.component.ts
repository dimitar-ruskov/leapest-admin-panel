...
@UntilDestroy()
export class SpCourseGeneralInfoComponent implements OnInit {
  ...
  selfPacedCourse: ActiveSelfPacedCourse;
  editLock = true;
  ...
  ngOnInit(): void {
    this.selfPacedCourse$
      .pipe(
        filter((resource: DeferredResource<ActiveSelfPacedCourse>) => !resource.isPending && !!resource.response),
        map((resource: DeferredResource<ActiveSelfPacedCourse>) => resource.response),
        untilDestroyed(this),
      )
      .subscribe((selfPacedCourse: ActiveSelfPacedCourse) => {
